/*
 * name: 			客户端主界面
 * description：		实现客户端主界面布局及逻辑处理
 * author:			李海
 */

package auction.client.view;

import auction.client.view.*; 
import auction.bean.*;
...
public class clientPanel extends JFrame implements ActionListener, MouseListener, WindowListener,ItemListener {
	...
	// 窗口菜单
	JLabel orderManage, home, bidManage, systemManage;
...
//	}
	...
	public void initTopMenu() {
		...
		String [] nameStrings = { "商    城", "竞     拍", "订单管理","系统管理"}; 
	
		home = CreateMenuLabel(home, nameStrings[0], jMenu);
		home.setName("home"); 
		home.setForeground(Color.yellow); 
		
		
		bidManage = CreateMenuLabel(bidManage, nameStrings[1], jMenu);
		bidManage.setName("bidManage");
		
		orderManage = CreateMenuLabel(orderManage, nameStrings[2], jMenu);
	...
	public void initCenter() {
		...
		orderManagePanel orderManagePanel = new orderManagePanel();		
		homePanel homePanel = new homePanel(this);	
		//bidGoods bidManagePanel = new bidGoods(null,0);	
		systemManagePanel systemManagePanel=new systemManagePanel();	

		conjp.add(homePanel, "home");
		//conjp.add(bidManagePanel, "bidManage");
	...
	public clientPanel() {		
		...
		AWTUtilities.setWindowOpacity(clientPanel.this, 0f);
	...
	// 设置
	// 定义顶层菜单样式函数
	public void setTopMenuStyle(JLabel jlb, String type) {		
		...
		if (type.equals("click")) {			
			String [] name = {"orderManage", "home", "bidManage", "systemManage"};
			JLabel [] jlb1 = {orderManage, home, bidManage, systemManage};
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getSource() == this.home) {			
			setTopMenuStyle(home, "click");
			this.update(this.getGraphics());
			this.card.show(conjp, "home");
		}		
		if (e.getSource() == this.bidManage) {			
			setTopMenuStyle(bidManage, "click");
			this.card.show(conjp, "bidManage");
		}		
	...
	public void mouseEntered(MouseEvent e) {
		...
		if (e.getSource() == this.home) {			
			setTopMenuStyle(home, "enter");
		}
		if (e.getSource() == this.bidManage) {			
			setTopMenuStyle(bidManage, "enter");
		}
	...
	public void mouseExited(MouseEvent e) {
		...
		if (e.getSource() == this.home) {			
			setTopMenuStyle(home, "exit");
		}
		if (e.getSource() == this.bidManage) {			
			setTopMenuStyle(bidManage, "exit");
		}
		if (e.getSource() == this.systemManage) {			
		...
		ActionListener lisener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (alpha < 0.9) {
					...
					AWTUtilities.setWindowOpacity(clientPanel.this, alpha+=0.1);
				}
				else {
					AWTUtilities.setWindowOpacity(clientPanel.this, 1);
